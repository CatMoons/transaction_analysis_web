...
def rep_category_spending(transactions, name_category, date):
    ...
    try:
        ...
        category_spending = transactions[
            (transactions['Категория'] == name_category) &
...
def rep_spending_on_weekdays(transactions, date):
    ...
    try:
        ...
        weekday_spending = transactions[transactions['Дата операции'] <= date].groupby('День недели')['Сумма операции'].sum()
...
def rep_spend_on_working_or_weekends(transactions, date):
    ...
    try:
        ...
        spending_by_day_type = transactions[transactions['Дата операции'] <= date].groupby('Тип дня')['Сумма операции'].sum()
